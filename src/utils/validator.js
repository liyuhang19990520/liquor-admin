...
var isWord = (rule, value, callback) => {
  if (value && !validate.isWord(value)) {
...
var istwoNull = (rule, value, callback) => {
  if (value && !validate.isTwoNumber(value)) {
    callback(new Error('必须为数值且小数点保留两位小数!'))
  } else {
    callback()
  }
}
